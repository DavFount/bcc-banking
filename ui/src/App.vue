...
const onMessage = (event) => {
  switch (event.data.type) {
    case 'toggle':
      ...
      sessionStore.setCharId(event.data.charId);
      accountStore.storeAccounts(event.data.accountData);
...
const closeApp = () => {
  visible.value = false;
  sessionStore.setBankId(null);
  sessionStore.setBankName(null);
  sessionStore.setCharId(null);
  accountStore.storeAccounts(null);
  api
    .post('updatestate', {
      state: visible.value,
    })
...
<template>
  <div class="container" v-if="visible || devmode">
    ...
    <div class="pl-64 h-full content">
      <div @click="closeApp" class="absolute text-gray-400 text-2xl right-3 hover:text-red-800">
        &times;
      </div>
      <div class="text-center">
        <h1 class="text-gray-50 title text-center text-3xl pt-5 font-crock">
          {{ `${getBankName} Bank` }}
        </h1>
      </div>
      <RouterView />
    ...
    <ModalPanel :visible="showCreateAccountModal" title="New Account">
      <div>
        <label for="accountName" class="block text-sm font-medium leading-6 text-gray-200"
          >Account Name</label
        >
        <div class="mt-2">
          <input
        ...
        <div class="flex justify-end">
          <button
            ...
            :class="{
              disabledButton: accountName === null || accountName.length < 5,
            }"
          >
            Create
          </button>
